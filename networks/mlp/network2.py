...
class Network(object):
    ...
    def default_weight_initializer(self):
        ...
        self.weights = [np.asmatrix(np.random.randn(y, x)/np.sqrt(x)) for x, y in zip(self.sizes_of_layers[:-1], self.sizes_of_layers[1:])]

    def large_weight_initializer(self):
        self.biases  = [np.asmatrix(np.random.randn(y, 1)) for y in self.sizes_of_layers[1:]]
        self.weights = [np.asmatrix(np.random.randn(y, x)) for x, y in zip(self.sizes_of_layers[:-1], self.sizes_of_layers[1:])]
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for epoch in range(epochs):
            ...
            if monitor_training_accuracy:
                ...
                print( "Accuracy on training data: {} / {} = {}".format(int(accuracy * 1.5), n, float(int(accuracy * 1.5)) / float(n)))
                print()
            ...
            if monitor_evaluation_accuracy:
                ...
                print( "Accuracy on evaluation data: {} / {} = {}".format(accuracy, n_data, float(accuracy) / float(n_data)))
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        self.weights = [(1-eta*(lmbda/n))*w-(eta/mini_batch_size)*nw for w, nw in zip(self.weights, nabla_w)]
